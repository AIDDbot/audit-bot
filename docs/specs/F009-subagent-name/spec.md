...
F003 persists subagent identity only from the subagent-start and subagent-stop rows of [`docs/normalized-fields.md`](../../normalized-fields.md), under the name `agent_type`, and only when `harness` and `event` match that table. Unmapped or header-only documents get no body fields. Prompt, agent-stop, and session-start documents therefore never persist the name even when the payload carries it. F004 then shows that identity only on start/stop rows, as `{name}: {value}` pairs (`agent_type: builder`). Developers scanning the report cannot tell which subagent ran on other rows, and the prefix is noise — they need the name only.
...
This spec does not replace F001–F008. Event log verbatim rules, Session index, observe-only exit/stdout, F002 positionals, F003 append-only / compact header / turn, F008 turn numbering, hook registrations, and F007 `agent_display_name` stay in force. This spec does not duplicate F003, F004, or F007 acceptance criteria except where those specs are amended for this field. F003 is amended so `subagent` may appear after the header on any document, including header-only/unmapped, without opening other extra body fields. F004 is amended so the Subagent cell is the bare `subagent` value when that field is present on the document. F007 is amended so identity is `subagent`, not `agent_type`; `agent_display_name` remains Copilot-only and is not a fallback for identity.

Normalized body fields per event kind remain those in [`docs/normalized-fields.md`](../../normalized-fields.md). Event-kind names across harnesses are those in [`docs/events-args.md`](../../events-args.md). That mapping table currently lists identity only on subagent start/stop; this spec requires renaming that row to `subagent` and persisting the field on every YAML document when a matching payload attribute is present.

### User Stories

- As a developer, I want **`subagent` persisted on every Session YAML log document when the payload names a subagent** so that identity is not limited to start/stop rows.
...
- The normalized mapping in [`docs/normalized-fields.md`](../../normalized-fields.md) must **rename** `agent_type` to `subagent`. Source keys for the subagent-start and subagent-stop rows stay: Cursor `subagent_type`; Copilot start `agentName`, Copilot stop `agentType`; Claude Code `agent_type`. An ingest must **not** keep writing `agent_type` on new YAML documents.
- A YAML document may **include `subagent` after the header and before other body fields** when a matching payload attribute is present, for every event kind (session start, session end, user prompt, agent stop, subagent start, subagent stop) and for header-only unmapped documents (`harness` or `event` empty or unmatched). Other body fields must **still follow** F003 table-driven mapping for the event kind.
- When no matching source attribute is present, `subagent` must **be omitted** (F003 omit-absent). When the matching source key is present and the value is `null`, `subagent` must **be YAML `null`**.
...
- `agent_display_name` must **still be persisted** on Copilot subagent start/stop YAML when Copilot sends `agentDisplayName` (F007). It must **not** appear in the report Subagent cell (F004).
- A Session report Subagent cell may **include only** the `subagent` value (the name), with no field-name prefix (`agent_type:`, `subagent:`, `agent_display_name:`, or similar). When `subagent` is absent from that document, the cell must **be empty**. The cell must **be filled whenever** that document has `subagent`, not only for start/stop. An ingest must **not** copy `subagent` onto later documents that omit it. An ingest must **not** reconstruct parent→subagent hierarchy. AC-F004.6 100-character single-line preview still applies to that cell. Details must **not** repeat `subagent` or the retired name `agent_type`, and must **not** include `agent_display_name`.
- An ingest must **not rewrite** previously written YAML documents (F003 append-only). Prior documents that still have `agent_type` are not migrated.
- JSONL must **stay verbatim** (F001). Session index, F002 positionals, compact YAML header (`harness`, `event`, `session_id` only on the initial session-start, `turn`), F008 turn numbering, hook registrations, and observe-only exit 0 / no blocking stdout must **not** change.
...
- Rewriting existing YAML documents or migrating old `agent_type` keys.
...
This spec does not add a persisted entity. It renames the YAML identity field `agent_type` → `subagent`, persists that field on every document when a matching payload attribute is present, and changes the report Subagent cell to that value only:
...
- **Session YAML log** — one `{session_id}.yaml` per distinct F001 session identifier (F003); new documents persist `subagent` after the header when a matching payload attribute is present, including header-only/unmapped; other body fields stay table-driven; prior documents are not rewritten.
- **Session report** — F004 downstream consumer; Subagent cell is the bare `subagent` value when that document has the field (F004 amend).
...
- On each YAML-appending ingest, persist `subagent` after the header when a preferred payload attribute is present, on every event kind including header-only/unmapped; omit when none of those attributes is present; write YAML `null` when the chosen key is present and null.
...
- Keep `agent_display_name` on Copilot subagent start/stop YAML when present (F007). Do not show it in the Subagent cell.
...
> last updated: 2026-09-02T09:50:00Z
