...
Ports: not applicable (no HTTP). Data cleaned under `temp/e2e/` before the run. Suite: `node --test e2e/*.test.ts` (78 pass, 0 fail — 11 F006 + 25 F004 + 7 F005 + 21 F003 + 5 F002 + 9 F001 regression). CLI units (extra signal): `cd cli && bun run test` (126 pass, 0 fail).
...
- [x] **AC-F006.3** — pass — `e2e/ac-f006.3-stop-yaml-header-only.test.ts` — `AC-F006.3 — stop YAML is F003 header then empty body; transcript_path omitted`
