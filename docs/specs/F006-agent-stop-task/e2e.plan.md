...
Grounding (F008 amend/replan of F006 e2e; prior plan last updated 2026-09-01T12:15:00Z):
...
- Each `node:test` title must carry the AC id (e.g. `AC-F006.8 — …`). Do **not** leave an AC-F006.3 title
...
- AC-F006.8 must assert: header keys `session_id`, `source_harness`, `source_event`, `timestamp`, `turn` then empty body; `turn` is a YAML integer (unquoted decimal, match `/^-?\d+$/`; must **not** be a quoted string); Event log has no `turn`. Do **not** assert incrementing, prompt-kind counting, or turn 1 on prompts (F008). Prefer `/^-?\d+$/` over exact `0` so F008 numbering later does not break these tests
- Existing `e2e/ac-f006.1` / `.2` / `.4` / `.5` / `.6` / `.7` files already slice five header keys where they assert header order (F003 sibling fix). Keep them. `.5` / `.6` already use `keys.slice(0, 5)` including `turn` and `keys.slice(5)` for the body. Do not add F008 numbering asserts to those files
- Do **not** break F001–F005 spawn tests except those already updated for six hook keys (Step 8 of the prior plan, already shipped). Do **not** plan F004 report-trigger e2e (sessionEnd gate, duration, overwrite). Sibling Builder amends F004 e2e
...
| AC-F006.3 — stop YAML starts with F003 header then empty body; transcript_path omitted | drop | retired; five-field header including `turn` is AC-F006.8. Replace `e2e/ac-f006.3-stop-yaml-header-only.test.ts` |
| AC-F006.4 — normalized-fields.md includes task for subagent start (Cursor only) | keep | mapping-table `task` exception unchanged; does not slice header keys |
| AC-F006.5 — Cursor subagentStart YAML includes task after agent_type when present, omitted when absent | keep | `task` after `agent_type` unchanged; file already slices five header keys (F003 sibling fix); do not assert F008 numbering |
| AC-F006.6 — Copilot and Claude Code subagentStart YAML omit task | keep | omit `task` unchanged; file already slices five header keys (F003 sibling fix); do not assert F008 numbering |
| AC-F006.7 — stop ingest and task include/omit stay observe-only | keep | observe-only unchanged; body already `keys.slice(5)`; do not assert F008 numbering |
...
New scenarios (not in the prior plan): **AC-F006.8** (stop YAML starts with five fields including unquoted YAML integer `turn`; empty body; no `transcript_path`; Event log has no `turn`).
...
### Step 3: AC-F006.8 — stop YAML starts with five-field F003 header then empty body; transcript_path omitted
New (replaces dropped AC-F006.3). Header is `session_id`, `source_harness`, `source_event`, `timestamp`, `turn` in that order; `turn` is an unquoted YAML integer (`/^-?\d+$/`); agent-stop body fields in [`docs/normalized-fields.md`](../../normalized-fields.md) are none today so the document is header-only after those five; `session_id` is not duplicated in the body; `transcript_path` is omitted even when the payload has a transcript path (F005); Event log has no `turn`. Do not assert F008 numbering. One Cursor case is enough (Copilot `agentStop` / Claude `Stop` aliases stay in cli unit tests). Verifies AC-F006.8.
- Paths:
    - `e2e/spawn.ts`
...
Keep. Parse [`docs/normalized-fields.md`](../../normalized-fields.md) (do not spawn ingest). Subagent-start table includes `task` with Cursor source key `task` and no Copilot or Claude Code source key, as an explicit exception to the three-harness intro. Verifies AC-F006.4.
- Paths:
...
Keep. Two cases in one AC file: present `task` and absent `task`. File already slices five header keys (F003 sibling fix). `task` follows `agent_type` when the mapped Cursor source key is present; omit when absent. Do not assert F008 numbering. One Cursor case is enough. Verifies AC-F006.5.
- Paths:
    - `e2e/spawn.ts`
...
- [x] Act: spawn both cases via `node cli/src/index.ts ingest cursor subagentStart` (do not import `cli/src/**`)
- [x] Assert: both `exitCode === 0`; stdout empty. Both documents start with keys `session_id`, `source_harness`, `source_event`, `timestamp`, `turn` in that order (`keys.slice(0, 5)`); `source_harness` is `cursor`; `source_event` is `subagentStart`; `session_id` equals the filename stem and appears once. Case 1: body keys are `agent_type` then `task` (`keys.slice(5)`); `agent_type` is `"explore"`; `task` is `"review the diff"`; extras absent from YAML (`transcript_path` substring absent). Case 2: body keys are `agent_type` only (no `task`). Event log line remains verbatim including extras and, in case 1, `task` (AC-F006.5). Do not assert F008 numbering

---

### Step 6: AC-F006.6 — Copilot and Claude Code subagentStart YAML omit task
Keep. Spawn ingest as extra argv only (do not spawn Copilot or Claude processes). File already slices five header keys (F003 sibling fix). Neither harness maps `task`; a payload key `task` or any other decoy field must not become YAML `task`. Do not assert F008 numbering. Verifies AC-F006.6.
- Paths:
    - `e2e/spawn.ts`
...
- [x] Assert: both `exitCode === 0`; stdout empty. Both documents start with the five F003 header keys (`keys.slice(0, 5)`); `source_harness` / `source_event` match the extra argv (`copilot` / `subagentStart`, `claude-code` / `SubagentStart`). Body has `agent_type` and **no** `task` (not from the trap `task` key, not from `agentDescription` / `agent_id` or any other field). Event log line remains verbatim including the trap `task` key (AC-F006.6). Do not assert F008 numbering

---

### Step 7: AC-F006.7 — stop ingest and task include/omit stay observe-only
Keep. Those spawns (a `stop` ingest, a Cursor subagentStart with `task`, and a Cursor subagentStart without `task`) exit 0 with empty stdout. No continue/block/permission/followup rewrite on stdout. Body already `keys.slice(5)`. Do not assert F008 numbering. Verifies AC-F006.7.
- Paths:
    - `e2e/spawn.ts`
...
- e2e later `/codify`: skipped typecheck, lint, and `node --test e2e/*.test.ts` (no e2e tsconfig/oxlint; skill forbids running the e2e suite). Keep `.1` `.2` `.4` `.5` `.6` `.7`. Rename `.3` → `.8` with AC-F006.8 titles. Do not edit `cli/src`, `spec.md`, or spec status.
