...
- As a developer, I want **each YAML document to carry harness, source event, and a local time** so that the document is self-contained even though the filename and the daily folder already encode session and date.
...
- An ingest must **not rewrite, reorder, or restructure** previously written documents in a Session YAML log (append-only).
...
- A YAML document always **starts with these four fields, in this order**: `session_id`, `source_harness`, `source_event`, `timestamp`.
...
- When `source_harness` or `source_event` does **not match** a mapping row and column, the document must **contain the four header fields only**.
...
- **Session YAML log** — one `{session_id}.yaml` per distinct F001 session identifier; append-only multi-document YAML; each document is one Event, normalized.
...
- Build that YAML document from the in-memory event plus the F002 source harness and source event positionals; do not re-read the Event log or Session index to produce it.
...
> last updated: 2026-09-01T09:45:00Z
