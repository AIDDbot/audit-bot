...
Grounding (first e2e plan for F003):
...
- Helper: `e2e/spawn.ts` — `spawnIngest` already accepts optional `extraArgv` after `"ingest"` (default none). F001 tests (AC-F001.1–7) rely on default none and remain valid. **Do not break F001 or F002 spawn tests.** Extend with YAML path helpers if needed; do not change the default `extraArgv` behavior
...
- Each `node:test` title must carry the AC id (e.g. `AC-F003.1 — …`)
...
> Copied verbatim from [cli.plan.md](./cli.plan.md). Event log, Session index, project root, and day folder stay as F001. Concurrency now covers the YAML append. Argv now passes harness/event into ingest for YAML only.
...
|----------------|--------|------|
| first | keep | First e2e plan for F003; no prior scenarios to classify |

## Implementation Steps

### Step 1: AC-F003.1 — Same invocation writes Event log, Session index, and one YAML document
Spawn ingest once as `ingest cursor sessionStart` with a JSON object that has a session identifier → F001 persist plus exactly one YAML document in `{session_id}.yaml` in the dated folder. One process, one invocation. Verifies AC-F003.1.
- Paths:
    - `e2e/spawn.ts`
...
Two sequential ingests for the same session identifier → the Session YAML log is multi-document YAML; each appended document begins with `---`; the first document is unchanged after the second append. Verifies AC-F003.2.
- Paths:
    - `e2e/spawn.ts`
...
### Step 3: AC-F003.3 — Document header is session_id, source_harness, source_event, timestamp
Spawn ingest with both positionals, and again with neither → every YAML document starts with those four keys in that order. Header `session_id` equals the F001 identifier and the filename stem. Positionals equal the argv values (empty string when omitted). Verifies AC-F003.3.
- Paths:
    - `e2e/spawn.ts`
...
Spawn ingest with a payload `timestamp`, and again without → YAML `timestamp` is host-local zero-padded `HH:MM:SS`. A generated value is not written onto the Event log line. Verifies AC-F003.4.
- Paths:
    - `e2e/spawn.ts`
...
Spawn ingest with known harness/event positionals → YAML body has only the normalized fields for that event kind (excluding `session_id`), mapped from [`docs/normalized-fields.md`](../../normalized-fields.md) source keys, in table order. Absent source keys are omitted; present `null` is YAML `null`; harness-specific extras stay off the YAML document. Verifies AC-F003.5.
- Paths:
    - `e2e/spawn.ts`
...
- [x] Arrange: isolated fixtures + `CURSOR_PROJECT_DIR`. Do not spawn Copilot or Claude processes; pass mapping names on argv. Each payload that must produce a YAML file includes a F001 session identifier (`session_id` or `conversation_id`). Cases (each title includes `AC-F003.5`):
    ...
    4. Present null — extra argv `["cursor", "subagentStart"]`; payload `transcript_path: null` and `subagent_type` set. Body has `transcript_path` as YAML `null` and `agent_type` as a scalar
...
Spawn a session-start ingest then a subagent-start ingest that share a F001 session identifier → the Session YAML log holds two independent sequential documents; the subagent event is not nested under the parent. Verifies AC-F003.6.
- Paths:
    - `e2e/spawn.ts`
...
Spawn ingest with a payload that has no F001 session identifier (only Copilot `sessionId`) → Event log line exists; Session index unchanged; no `{dayFolder}/*.yaml`. Verifies AC-F003.7.
- Paths:
    - `e2e/spawn.ts`
...
### Step 8: AC-F003.8 — Unrecognized harness/event: header-only YAML document
Spawn ingest with unrecognized positionals and a F001 session identifier → a YAML document is still appended, containing the four header fields only. Verifies AC-F003.8.
- Paths:
    - `e2e/spawn.ts`
...
Two overlapping ingest processes plus a sequential repeat → complete YAML documents (each starts with `---`); Event log and Session index stay valid as F001. Same overlap pattern as `e2e/ac-f001.5-concurrent-persist.test.ts`. Verifies AC-F003.9.
- Paths:
    - `e2e/spawn.ts`
...
Read `cli/package.json` and spawn the existing ingest entry → Node ≥ 24 ESM, `dependencies` empty, no new binary. Verifies AC-F003.10.
- Paths:
    - `e2e/spawn.ts`
...
- No `docs/arch/e2e.arch.md` — architecture has no e2e product container; this suite is the repo-root spawn folder from AGENTS.md. Architecture link is [`system.arch.md`](../../arch/system.arch.md), same as F002.
- Did not run `node --test e2e/*.test.ts` (planify only; later e2e codify is compile/lint only). No e2e `tsconfig` and no e2e oxlint config, so typecheck and lint are skipped (same as F001/F002).
- No HTTP, no ports, no database. `/verify` "free the ports" does not apply.
- `spawnIngest` already has optional extra argv; omitting it keeps F001 spawn tests on argv `["ingest"]` only. This plan only adds YAML path helpers; it does not change the helper’s default.
- YAML in tests is observed as text (split on `---`, read keys in order). No YAML library in e2e either.
- Current [`cli.arch.md`](../../arch/cli.arch.md) still says source harness/event are not passed into ingest. F003 e2e asserts YAML headers from those positionals; the architecture update is the cli sibling’s concern. Do not invent an e2e architecture file to paper over that.
...
- Do not register `beforeSubmitPrompt` / `stop` (or any extra Cursor event). Prompt and agent-stop mapping are spawned as ingest extra argv only.
- Do not revive `.cmd` wrappers (AGENTS.md learning scar). Cursor registration is unchanged from F002.
- This container does not set spec status to `planned` (sibling cli planify runs in parallel).
