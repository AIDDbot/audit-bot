...
- [x] Keep the unmapped initial sessionStart (`includeSessionId: true`, unrecognized harness or event) **with** `subagent_type` case: five header fields then `subagent`; extra table fields (`reason`) stay omitted (AC-F003.16, AC-F003.17)
- [x] Add AC-F003.18 emitter coverage: `emitSessionRecord` output is one `JSON.parse`-able object (not a YAML document: no leading `---`, no `source_harness`). Mapping (header keys, optional `session_id`, body / `subagent`) applies to that object. Do not restate F010 filename or lock here (AC-F003.18)
- [x] Keep compact-header, `isInitialSessionStart`, timestamp (AC-F003.4), Copilot/Claude mapping, and F006 header-only-without-identity tests. Do not change F008 `nextConversationTurn` / `isInitialSessionStart` formulas or their F010 scanner fixtures
...
> last updated: 2026-09-02T15:45:00Z
