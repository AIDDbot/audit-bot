...
User-facing flow under test: ingest (spawned as `ingest {harness} {event}`) persists F001 Event log / Session index and F003 Session YAML. Each appended YAML document’s fifth header field `turn` is an unquoted YAML integer: the count of prompt-kind documents already in that session’s Session YAML log, plus one when this document is itself prompt-kind; otherwise that same count; `0` when none are present and this document is not prompt-kind. Prompt-kind is only the F002 positional `source_event` values `beforeSubmitPrompt`, `userPromptSubmitted`, and `UserPromptSubmit`. Repeated `stop` / `agentStop` / `Stop` / `subagentStop` / `SubagentStop` stay on the current turn. Prior documents are not rewritten. The Event log stays F001 verbatim (no `turn` overlay, no sidecar Turn file). Observe-only: exit 0, no blocking stdout. F008 does not change Cursor registration (six events stay). Do not add Copilot or Claude registrations. Do not add `.cmd` wrappers.

This spec does not replace F001–F007. How the fifth header field is present and typed as a YAML integer is F003 (AC-F003.11 / AC-F003.12). Session report grouping, turn duration, and prompt-in-subsection are F004 (`e2e/ac-f004.17-turn-subsections.test.ts`, `e2e/ac-f004.18-turn-duration.test.ts`, `e2e/ac-f004.19-turn-prompt.test.ts`) — do not reopen. This is a functional-spec extra run. Architecture has **no e2e product container** — do not invent `e2e.arch.md` and do not rename `cli-node`. Scenarios live in a thin `e2e/` folder of `node:test` files that **spawn** `cli/src/index.ts`. They must not import `cli/src/**` as the system under test. They must not spawn `.agents/hooks/index.mjs`. This plan carries **no unit tests** and plans no `cli/test/` work. This plan does **not** cover F004 report-trigger e2e.
...
Grounding (first e2e plan for F008):
...
- Do **not** reopen F001–F007 ACs. F003.11 / F005.6 already use `assertYamlIntegerTurn` (any integer, not exact `0`). F004.17 / F004.18 / F004.19 already cover report turn subsections / duration / prompt-in-subsection. Plan Step 7 as leave-as-is
...
|----------------|--------|------|
| first | keep | First e2e plan for F008; no prior scenarios to classify |

## Implementation Steps

### Step 1: AC-F008.1 — sequential sessionStart, prompt, stop numbers turn 0 then 1 then 1
Spawn three sequential ingests for the same `session_id`: `sessionStart` (not prompt-kind, none already present → `turn: 0`); `beforeSubmitPrompt` (prompt-kind, zero already present → `turn: 1`); `stop` (not prompt-kind, one already present → `turn: 1`). Assert unquoted YAML integers via `yamlRawScalar` / `assertYamlIntegerTurn` (must **not** be `"0"` / `"1"` quoted). Do not use `yamlMapping` values for `turn` (it strips quotes). Verifies AC-F008.1.
- Paths:
    - `e2e/spawn.ts`
...
- [x] Arrange: isolated fixture; same F001 `session_id` `"sess-ac-f008-2"` on every payload (Copilot `sessionId` alone is not a session identifier). Sequential extra argv after the first prompt (each later title includes `AC-F008.2`):
    ...
    4. Cursor `["cursor", "subagentStop"]` → `turn: 1`
    5. Copilot `["copilot", "agentStop"]` → `turn: 1`
    6. Claude `["claude-code", "Stop"]` → `turn: 1`
    7. Claude `["claude-code", "SubagentStop"]` → `turn: 1`
...
Sequence on one session: `sessionStart` then `subagentStart` (both preamble → `turn: 0`); first `beforeSubmitPrompt` → `turn: 1`; second `beforeSubmitPrompt` → `turn: 2`. Verifies AC-F008.3.
- Paths:
    - `e2e/spawn.ts`
...
Capture the first document’s bytes after `sessionStart` (`turn: 0`). After a later prompt then stop, the first document text (including `turn: 0`) is unchanged. No rewrite of prior `turn`. Verifies AC-F008.4.
- Paths:
    - `e2e/spawn.ts`
...
Prompt ingest: Event log line deep-equals the payload and has **no** `turn` key. No sidecar Turn file. Day-folder files besides jsonl / index / md / lock are only `{session_id}.yaml`. Prompt ingest stays observe-only. Verifies AC-F008.5.
- Paths:
    - `e2e/spawn.ts`
...
`sessionStart`, `beforeSubmitPrompt`, and `stop` all `exitCode === 0` and stdout `""` (no continue/block/permission JSON). Existing ESM ingest: spawn `cli/src/index.ts` via `spawnIngest` (not `.agents/hooks/index.mjs`). `cli/package.json` stays Node ≥ 24 ESM with empty `dependencies`. Do not edit `.cursor/hooks.json`. Verifies AC-F008.6.
- Paths:
    - `e2e/spawn.ts`
...
Leave existing F001–F007 e2e files as-is. F003.11 / F005.6 already use `assertYamlIntegerTurn` (any integer — F008 numbering must **not** break them). Do not reopen F004 turn-subsection / duration / prompt-in-subsection tests. Hook-key tests stay six events. F008 adds **no** Cursor registration. Do not plan F004 report-trigger e2e.
- Paths:
    - `e2e/ac-f003.11-yaml-document-header.test.ts`
    - `e2e/ac-f003.12-unrecognized-header-only.test.ts`
...
- [x] Assert:
    ...
    - `e2e/ac-f004.17-turn-subsections.test.ts`, `e2e/ac-f004.18-turn-duration.test.ts`, `e2e/ac-f004.19-turn-prompt.test.ts` — F004 report consumer; do not reopen
