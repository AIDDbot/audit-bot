...
Ports: not applicable (no HTTP). Data cleaned under `temp/e2e/` before the run. Suite: `node --test e2e/*.test.ts` (67 pass, 0 fail — 7 F005 + 25 F004 + 21 F003 + 5 F002 + 9 F001 regression). CLI units (extra signal): `cd cli && bun run test` (110 pass, 0 fail).
...
- [x] **AC-F005.3** — pass — `e2e/ac-f005.3-prompt-yaml-header-and-body.test.ts` — `AC-F005.3 — present prompt YAML is F003 header then prompt`; `AC-F005.3 — absent prompt YAML is header-only after the four fields`
...
> last updated: 2026-09-01T11:41:50Z
