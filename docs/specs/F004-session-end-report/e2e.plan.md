...
Grounding (amend/replan of the F004 e2e plan — compact YAML keys `harness` / `event`; overview `session_id` from F001 filename stem; AC-F004.21 / .22 / .23):
...
- Each `node:test` title must carry the AC id (e.g. `AC-F004.23 — …`)
...
- Do not add or remove Cursor hooks in this plan. Do not spawn Copilot or Claude processes. Prompt and agent-stop mapping may still be tested by spawning ingest with those event names so YAML (and thus the per-turn table) contains those kinds. F004.14 / F004.22 / F004.20 may spawn `stop` or include Cursor `task` as fixtures; F006 owns stop registration and the `task` mapping-table exception
...
> Session YAML log compact header copied from F003 (spec F003: `harness` / `event`; `session_id` only on the initial session-start) so e2e Arrange matches what ingest writes. Event log, Session index, project root, and day folder stay as F001/F003. F008 numbering is already shipped (0.14.0); this amend reads `turn`. Session report is written after every YAML append. Argv does not gate the report. Report labels follow YAML `harness` and `event`.
...
- When `harness` or `event` does not match a mapping row and column, the document contains the header fields only: five fields (`session_id`, `harness`, `event`, `timestamp`, `turn`) when it is the initial session-start; four fields (`harness`, `event`, `timestamp`, `turn`) otherwise.
- Node builtins only: no YAML library. Do not migrate or rewrite old YAML that still has `source_harness` / `source_event` / per-document `session_id`. Reading mixed historical files is out of scope.
...
| AC-F004.2 — Report table order matches YAML file order, not timestamp sort | redo | File order stays within each turn table. Existing file asserts YAML `values.source_event`. Flip to `values.event`. Keep Turn 0 = sessionStart, Turn 1 = prompt then sessionEnd (F008 fixture). Four-column header already. Do not assert `session_id` on later YAML docs |
...
| AC-F004.4 — Event-count summary: total documents and per-source_event counts | drop | Deprecated (v0.15.0): counts and table header use `event` (AC-F004.21). Delete `e2e/ac-f004.4-event-count-summary.test.ts` |
| AC-F004.5 — Details are mapped normalized body fields only | drop | Deprecated (v0.11.0): events grouped by turn (AC-F004.22). Matching test already gone. Stay drop |
| AC-F004.17 — One subsection per distinct turn; Details in each turn table | drop | Deprecated (v0.15.0): Event column and Details mapping key off `event` (AC-F004.22). Delete `e2e/ac-f004.17-turn-subsections.test.ts` |
| AC-F004.20 — Subagent filled only on subagent start/stop rows | keep | Existing file does not hardcode `source_harness` / `source_event` or report labels. Subagent fill / empty / no-inheritance intent unchanged |
| AC-F004.18 — Turn-0 duration is first turn-0 timestamp → last turn-0 timestamp | keep | Existing file does not hardcode those YAML/report labels. Turn-duration intent unchanged |
| AC-F004.19 — Turn 0 has no Prompt line; turn ≥ 1 Prompt uses 100-character preview | keep | Existing file does not hardcode those YAML/report labels. Prompt-line / 100-char intent unchanged |
| AC-F004.6 — Preview: 100-character limit, ellipsis, single line | keep | Existing `e2e/ac-f004.6-details-preview-100-chars.test.ts` does not hardcode those YAML/report labels. 100-char / Subagent truncation intent unchanged. Leftover `e2e/ac-f004.6-details-preview-80-chars.test.ts` if present: still drop (prior amend) |
| AC-F004.7 — Subagent start and stop are ordinary chronological rows | keep | Existing file does not hardcode those YAML/report labels. Ordinary-row / no-nest intent unchanged |
| AC-F004.8 — Session report is `{session_id}.md` Markdown tables, not HTML | keep | Existing file already asserts four-column header and no `<table>`. Does not hardcode `source_harness` / `source_event` |
| AC-F004.9 — Observe-only: exit 0 and empty stdout, including report failure | keep | Existing file does not hardcode those YAML/report labels. Failure fixture (`.md` as directory) still valid |
| AC-F004.10 — Existing Node ESM ingest, no extra runtime dependencies | keep | Existing file does not hardcode those YAML/report labels. Smoke spawn still valid |
| AC-F004.11 — Report is produced from YAML only, not Event log or Session index | redo | Tamper jsonl/index then later ingest; report still matches YAML. Existing file asserts YAML `values.source_event`. Flip to `values.event`. Overview `session_id` stays the F001 id. Four-column helper already |
...
| AC-F004.13 — No session identifier: F001 persist, no YAML, no Session report | keep | Existing file does not hardcode those YAML/report labels. Still: no yaml, no md; Copilot `sessionId` is not an identifier |
| AC-F004.14 — Same invocation writes YAML and Session report after any YAML-appending ingest | redo | Report-after-every-YAML-append still holds. Existing file asserts YAML `values.source_event`. Flip to `values.event`. Do not assert `session_id` on the stop-only YAML doc |
| AC-F004.15 — Overview session_id, source_harness from last document, start/end/duration | drop | Deprecated (v0.15.0): overview `harness`; `session_id` from F001 filename stem without requiring it on every YAML document (AC-F004.23). Delete `e2e/ac-f004.15-overview-times-and-duration.test.ts` |
| AC-F004.16 — Later YAML append same session same day overwrites `{session_id}.md` | keep | Existing file does not hardcode those YAML/report labels. Overwrite / Turn 0 / Turn 1 row counts still track YAML |

New scenarios (not in the prior plan as these AC ids): **AC-F004.21** (counts + `| event | count |`); **AC-F004.22** (turn tables Event column from `event`; same Details/Subagent/omit turn 0 as old .17); **AC-F004.23** (overview `session_id` from F001 filename even when YAML omits it; `harness` not `source_harness`; duration unchanged).

## Implementation Steps

### Step 1: AC-F004.2 — Report table order matches YAML file order, not timestamp sort
Several events for one session; each **turn** table’s rows follow YAML document file order even when payload timestamps would sort differently. Flip YAML `source_event` → `event` only. Verifies AC-F004.2.
- Paths:
    - `e2e/spawn.ts`
    - `e2e/ac-f004.2-report-table-file-order.test.ts`
- [ ] Arrange: one fixture; same payload `session_id` `"sess-ac-f004-2"` for every spawn (F001 identifier). Choose Unix-ms `timestamp` values whose host-local `HH:MM:SS` would sort in a **different** order than ingest order (same pattern as `e2e/ac-f003.4-timestamp-hhmmss.test.ts`: format with local `getHours` / `getMinutes` / `getSeconds`, zero-padded). Example order: (1) extra argv `["cursor", "sessionStart"]` with a **later** clock time (e.g. 12:00:00); (2) extra argv `["cursor", "beforeSubmitPrompt"]` with an **earlier** clock time (e.g. 10:00:00) and a `prompt` so the row is identifiable; (3) extra argv `["cursor", "sessionEnd"]` with a middle or later clock time (e.g. 11:00:00) and `reason`. Do not register `beforeSubmitPrompt`. Snapshot YAML document count and each document’s `timestamp` / `event` (via `yamlDocuments` + `yamlMapping.values.event`, not `source_event`) after the last spawn. First YAML document (sessionStart) has `session_id`; later documents omit it — do **not** assert `session_id` on every mapping. F008 will put sessionStart in Turn 0 and prompt + sessionEnd in Turn 1 — use that as a fixture; do not assert YAML `turn` integers
- [ ] Act: spawn the three ingests in that file order (title includes `AC-F004.2`)
- [ ] Assert: YAML has exactly three documents in ingest order (`event` `sessionStart`, `beforeSubmitPrompt`, `sessionEnd`) with the arranged timestamps; `{session_id}.md` exists; the report has `## Turn 0` then `## Turn 1` and does **not** contain `## Events`; the Turn 0 table has one data row (`12:00:00` `sessionStart`); the Turn 1 table has two data rows in **file** order (`10:00:00` `beforeSubmitPrompt`, then `11:00:00` `sessionEnd`) — not sorted by Time; each turn table header is exactly `| Time | Event | Subagent | Details |`; each data row has four cells; Subagent is empty on these rows (AC-F004.2)

---

### Step 2: AC-F004.21 — Event-count summary: total documents and per-event counts
Report includes the number of YAML documents and a breakdown of how many documents have each distinct `event` (session-level, not per-turn). Table header is `event`. Replaces AC-F004.4. Verifies AC-F004.21.
- Paths:
    - `e2e/spawn.ts`
    - `e2e/ac-f004.21-event-count-summary.test.ts`
    - delete `e2e/ac-f004.4-event-count-summary.test.ts`
- [ ] Arrange: one fixture; payload `session_id` `"sess-ac-f004-21"`. Sequential extra argv: `["cursor", "sessionStart"]`; `["cursor", "beforeSubmitPrompt"]` (with `prompt`); `["cursor", "beforeSubmitPrompt"]` again (repeat kind); `["cursor", "sessionEnd"]` (with `reason`). Four documents; `beforeSubmitPrompt` appears twice. Do not register `beforeSubmitPrompt`. Copy cases from `e2e/ac-f004.4-event-count-summary.test.ts`; flip the count-table lookup from `| source_event | count |` to `| event | count |`. Title `AC-F004.21` (no `AC-F004.4` title)
- [ ] Act: spawn the four ingests in order (title includes `AC-F004.21`)
- [ ] Assert: YAML has four documents; the report states total document count **4**; the counts table header is exactly `| event | count |` and is **not** `| source_event | count |`; per-`event` breakdown includes `sessionStart` 1, `beforeSubmitPrompt` 2, `sessionEnd` 1 (and no extra kinds) (AC-F004.21)

---

### Step 3: AC-F004.22 — One subsection per distinct turn; four-column turn tables; Event is YAML `event`
Spawn several events for one session → report has one `## Turn {n}` per distinct turn that appears, does **not** contain a session-wide `## Events` heading, each subsection table has exactly columns Time, Event, Subagent, Details. Event is YAML `event` (not `source_event`). Details are the remaining `docs/normalized-fields.md` body fields excluding `session_id` and excluding identity (`agent_type` / `agent_display_name`). Same Details / Subagent / omit-turn-0 cases as old AC-F004.17. Keep Details / Subagent values ≤100 characters so truncation is out of this AC (that is AC-F004.6). Subagent **fill rules** are AC-F004.20 — this step only needs identity **not** in Details, and Details for subagent start = `task` only. Do not assert F008 numbering. Replaces AC-F004.17. Verifies AC-F004.22.
- Paths:
    - `e2e/spawn.ts`
    - `e2e/ac-f004.22-turn-subsections.test.ts`
    - delete `e2e/ac-f004.17-turn-subsections.test.ts`
- [ ] Arrange: isolated fixtures + `CURSOR_PROJECT_DIR`. Copy cases from `e2e/ac-f004.17-turn-subsections.test.ts`; change every title `AC-F004.17` → `AC-F004.22`. Event column values stay the event-name strings; they come from YAML `event`. Do not spawn Copilot or Claude processes; pass mapping names on argv. Each payload that must produce YAML includes a F001 session identifier. Keep values ≤100 characters. Do **not** plan F006 mapping-table exception cases as a separate AC. Local `eventRows` already looks up `| Time | Event | Subagent | Details |` and four cells — keep that. Cases (each title includes `AC-F004.22`; **no AC-F004.17 title**):
...
- [ ] Assert: no `## Events`; each turn table header is exactly `| Time | Event | Subagent | Details |`; Event cells equal YAML `event`; each data row has four cells; Details use snake_case names in table order, `{name}: {value}` pairs separated by `; ` when multiple; subagent start Details are `task` only; subagent stop Details are `response_text` only; `agent_type` / `agent_display_name` do not appear in Details; omitted when absent; empty for sessionStart, agent stop, and header-only; YAML `null` appears; `session_id` not in Details; `|` stays inside one cell; prompt-only spawn omits `## Turn 0` (AC-F004.22)

---

### Step 4: AC-F004.20 — Subagent filled only on subagent start/stop rows
Keep. Subagent cell is filled only for subagent-start and subagent-stop documents; empty for every other event kind; later non-subagent rows do not inherit identity; both identity fields absent → empty Subagent. Do not change Subagent fill intent. Verifies AC-F004.20.
- Paths:
    - `e2e/spawn.ts`
...
- [x] Arrange: isolated fixtures + `CURSOR_PROJECT_DIR`. Do not spawn Copilot or Claude processes. Do not reconstruct hierarchy. Keep identity values ≤100 characters (truncation is AC-F004.6). Each title includes `AC-F004.20`. Cases:
    1. Mixed sequence — sequential extra argv: `["cursor", "sessionStart"]`; `["cursor", "subagentStart"]` with `subagent_type` (e.g. `"explore"`); `["cursor", "subagentStop"]` with `subagent_type`; `["cursor", "beforeSubmitPrompt"]` with `prompt`; `["cursor", "stop"]`; `["cursor", "sessionEnd"]` with `reason`. Same payload `session_id` throughout (subagent start may use `parent_conversation_id` as in AC-F004.7). F008 will put sessionStart + both subagent rows in Turn 0 and prompt + stop + sessionEnd in Turn 1 — use that as a fixture
...
Keep. A Details / Subagent / per-turn prompt value longer than 100 characters is the first 100 characters plus `...`; 100 or fewer gets no ellipsis; newlines become spaces **before** the limit. Do not change 100-char / Subagent truncation intent. Verifies AC-F004.6.
- Paths:
    - `e2e/spawn.ts`
...
Keep. sessionStart + subagentStart + subagentStop + sessionEnd → four ordinary rows **inside that turn’s table**; no nesting or indentation as children. Do not assert Subagent fill (AC-F004.20). Verifies AC-F004.7.
- Paths:
    - `e2e/spawn.ts`
...
After writing YAML via ingest, tamper `events.jsonl` and/or `sessions.json` so they disagree with YAML; then a later YAML-appending ingest; the report must match YAML documents, not the tampered jsonl/index. Flip YAML `source_event` → `event` only. Verifies AC-F004.11.
- Paths:
    - `e2e/spawn.ts`
    - `e2e/ac-f004.11-report-from-yaml-not-jsonl.test.ts`
- [ ] Arrange: one fixture; payload `session_id` `"sess-ac-f004-11"`. First spawn extra argv `["cursor", "sessionStart"]` with a distinctive `timestamp`. Then **tamper** on disk (do not go through ingest): append an extra JSONL line to `events.jsonl` whose payload would look like another event (e.g. a `beforeSubmitPrompt` with a unique `prompt` `"tampered-from-jsonl"`) and rewrite `sessions.json` to include an extra identifier (e.g. `"tampered-session"`) and/or omit the real id. Then spawn extra argv `["cursor", "sessionEnd"]` with `reason` and a second distinctive `timestamp`. YAML after that has exactly two documents (`sessionStart`, `sessionEnd`) — the tampered JSONL line was never appended as YAML. Snapshot YAML `event` via `yamlMapping.values.event` (not `source_event`). First YAML doc has `session_id`; the sessionEnd doc omits it — do **not** require `session_id` on both mappings. Four-column `eventRows` lookup already
- [ ] Act: spawn sessionStart; tamper jsonl and index; spawn sessionEnd (title includes `AC-F004.11`)
- [ ] Assert: YAML still has exactly two documents in file order (`event` `sessionStart`, `sessionEnd`); `{session_id}.md` turn table has exactly two data rows (`sessionStart`, `sessionEnd`) and does **not** contain `tampered-from-jsonl` or `tampered-session`; total document count in the report is 2, not 3; overview `session_id` is `"sess-ac-f004-11"` (F001 identifier / filename stem); table header is `| Time | Event | Subagent | Details |` (AC-F004.11)
...
Spawn ingest with a session identifier → F001 persist plus one YAML document and `{session_id}.md` in the dated folder, same process, same invocation, including when no session-end document is in the file. Also still true for `sessionEnd`. Flip YAML `source_event` → `event` only. Verifies AC-F004.14.
- Paths:
    - `e2e/spawn.ts`
    - `e2e/ac-f004.14-same-invocation-yaml-and-report.test.ts`
- [ ] Arrange: isolated fixtures under `{repo}/temp/e2e/`; env `CURSOR_PROJECT_DIR` pointing at each. Do not import `cli/src/**`. Do not spawn `.agents/hooks/index.mjs`. Do not register `stop`. Do not infer the report trigger from the payload. Read YAML `event` via `yamlMapping.values.event` (not `source_event`). Cases (each title includes `AC-F004.14`):
...
Report overview uses `session_id` equal to the F001 identifier / filename stem even when YAML omits `session_id` on later or non-start documents; `harness` from the **last** document (not `source_harness`); start = first YAML `timestamp`; end = last YAML `timestamp`; duration zero-padded `HH:MM:SS` first→last regardless of `event`. Must not require sessionEnd. Must not use `duration_ms`. Last-before-first and equal timestamps both yield `00:00:00`. Duration cases unchanged from old AC-F004.15. Replaces AC-F004.15. Verifies AC-F004.23.
- Paths:
    - `e2e/spawn.ts`
    - `e2e/ac-f004.23-overview-times-and-duration.test.ts`
    - delete `e2e/ac-f004.15-overview-times-and-duration.test.ts`
- [ ] Arrange: four isolated fixtures. Payload `timestamp` is Unix-ms so YAML `HH:MM:SS` is deterministic (do not rely on generate-on-receive). Do **not** use `sessionEnd` / `SessionEnd` as the last (or only) event except where a case is prompt-only. Copy duration cases from `e2e/ac-f004.15-overview-times-and-duration.test.ts`; flip YAML `values.source_event` → `values.event`, `values.source_harness` → `values.harness`, and overview `| source_harness |` → `| harness |`. Titles `AC-F004.23` (no `AC-F004.15` title). Cases (each title includes `AC-F004.23`):
...
- Session YAML log compact-header wording is taken from F003 spec (`harness` / `event`; `session_id` only on the initial session-start), not copied verbatim from the sibling [cli.plan.md](./cli.plan.md) (that sibling still describes the old five-field `source_harness` / `source_event` / per-document `session_id` header). E2e Arrange must match what ingest writes after F003.
- F008 is released (0.14.0). E2e spawn can produce `## Turn 1`. This plan uses that as a fixture for AC-F004.22 / .19 / .20 / .2 / .16. It does **not** re-test F008 numbering ACs. Do not break F001–F003 or F005–F008 spawn tests.
...
- Drop of AC-F004.1 / AC-F004.3 / AC-F004.5 / AC-F004.12 already authorized deletion of those files (already gone). Drop of AC-F004.4 / AC-F004.15 / AC-F004.17 authorizes deleting `e2e/ac-f004.4-event-count-summary.test.ts`, `e2e/ac-f004.15-overview-times-and-duration.test.ts`, and `e2e/ac-f004.17-turn-subsections.test.ts`. Leftover `e2e/ac-f004.6-details-preview-80-chars.test.ts` if present is still drop (prior amend).

---

> last updated: 2026-09-02T08:25:00Z
