...
- [x] Assert: no `## Events`; each turn table header is exactly `| Time | Event | Subagent | Details |`; Event cells equal YAML `event`; each data row has four cells; Details use snake_case names in table order, `{name}: {value}` pairs separated by `; ` when multiple; subagent start Details are `task` only; subagent stop Details are `response_text` only; `subagent` / `agent_display_name` do not appear in Details (do **not** assert `agent_type` omission); omitted when absent; empty for sessionStart, agent stop, and header-only; YAML `null` appears; `session_id` not in Details; `|` stays inside one cell; prompt-only spawn omits `## Turn 0` (AC-F004.22)
