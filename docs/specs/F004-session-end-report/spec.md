...
This amend (F009) fills the Subagent cell with **only the `subagent` value** (the name) whenever that field is present on the YAML document — any event kind, not only start/stop — with no field-name prefix (`agent_type:`, `subagent:`, `agent_display_name:`, or similar). `agent_display_name` stays in YAML per F007 when Copilot sends it and stays out of Details; it is not shown in the Subagent cell. Identity is not copied onto later rows that omit it. Parent→subagent hierarchy stays out of scope. YAML persistence of `subagent` on every event is F003 / F009; this spec does not duplicate those ACs. AC-F004.20 (prefixed `{name}: {value}` pairs on start/stop only) is retired.

This amend drops the session-end gate. A Session report must still be produced when `sessionEnd` never arrives (the session ended abruptly). Duration stays elapsed clock time from the first YAML document’s `timestamp` to the last, whatever those events are — not Cursor `duration_ms` and not a session-end-only field. Overview `harness` comes from the last document (the ingest that just ran). YAML already persists `subagent`, `agent_display_name`, `task`, and `response_text` (F009 / F007 / F006 mapping; F003 omits absent fields).
...
This amend (released 0.13.0) adds a Subagent column so subagent identity is not packed into Details. Each per-turn event table has four columns, in this order: Time, Event, Subagent, Details. Subagent is optional: it is filled whenever that YAML document has `subagent`, as the bare name only (F009; AC-F004.24), and is empty when that field is absent. Details must not repeat `subagent` or `agent_display_name`. Preview length is 100 characters (first 100 + `...`) for Details cells, Subagent cells, and the per-turn prompt line. Parent→subagent hierarchy stays out of scope (`agent_id` is not in the YAML). Persistence of `subagent` on every event is F003 / F009; this spec is the report consumer.

This amend follows F003 compact YAML headers. Report labels use YAML `harness` and `event` (not `source_harness` / `source_event`). Overview `session_id` is always the F001 identifier (filename stem); the report must not require `session_id` on every YAML document. When the first document has `session_id`, it matches; when omitted, the report still shows the F001 identifier. Overview `harness` is the last document’s `harness`. Event-count and the Event column use `event`. Migrating or reading mixed historical YAML that still has `source_harness` / `source_event` / per-document `session_id` is out of scope.
...
- As a developer, I want **a Subagent column on each per-turn event table** so that I can see the subagent name when that document persisted it, as the name only, without packing identity into Details.
...
- As a developer, I want **report labels `harness` and `event` matching the YAML keys** so that overview, counts, and the Event column stay aligned with the compact header.
- As a developer, I want **overview `session_id` from the F001 identifier** so that the report still names the session when YAML omits `session_id` on later documents.
...
- A Session report must **include a session overview** with: `session_id` (always the F001 identifier / filename stem already used for that session); `harness` from the last document (the ingest that just ran); start time (first document’s `timestamp`); end time (last document’s `timestamp`); and total duration. Overview `session_id` must **not require** `session_id` on every YAML document. When the first document has `session_id`, it matches; when omitted, the overview must **still show** the F001 identifier. `harness` must **not** be taken only from a session-end document.
...
- Session duration is always **elapsed clock time** from the first YAML document’s `timestamp` to the last YAML document’s `timestamp` in that file, regardless of those documents’ `event`, displayed as zero-padded `HH:MM:SS`. An ingest must **not** use Cursor `duration_ms` or any session-end-only field for duration. When the last timestamp is **before** the first, duration must **be `00:00:00`**. When they are equal, duration must **be `00:00:00`**.
- A Session report must **include an event-count summary**: the number of YAML documents, and a breakdown of how many documents have each distinct `event` value (session-level, not per-turn). The table header must **be `event`**.
...
- A Subagent cell may **include only** that document’s `subagent` value (the name), with no field-name prefix (`agent_type:`, `subagent:`, `agent_display_name:`, or similar). Present values including YAML `null` appear as that value. A Subagent cell must **be filled whenever** that document has `subagent`, for any event kind. When `subagent` is **absent**, Subagent must **be empty**. `agent_display_name` must **not** appear in the Subagent cell (it stays in YAML per F007 when Copilot sends it). An ingest must **not** reconstruct parent→subagent hierarchy. An ingest must **not** copy `subagent` onto later documents that omit it.
- Details may **include only** the remaining normalized common body fields for that event kind in [`docs/normalized-fields.md`](../../normalized-fields.md), excluding `session_id` (already in the overview) and excluding `subagent` and `agent_display_name` (`subagent` belongs in the Subagent cell as the bare name; `agent_display_name` is not shown there), using those snake_case names, in table order, omitting fields absent from the document. Present values including YAML `null` appear. Multiple present fields in one cell are `{name}: {value}` pairs in table order, separated by `; `.
- Event kinds for that Details mapping are: session start (`sessionStart` / `SessionStart`) — Details empty; session end (`sessionEnd` / `SessionEnd`) — `reason`; subagent start (`subagentStart` / `SubagentStart`) — `task` only; subagent stop (`subagentStop` / `SubagentStop`) — `response_text` only; user prompt (`beforeSubmitPrompt` / `userPromptSubmitted` / `UserPromptSubmit`) — `prompt`; agent stop (`stop` / `agentStop` / `Stop`) — Details empty. When the document is header-only (F003 unmapped harness or event), Details must **be empty** (`subagent` when present is the Subagent cell, not Details). Cursor and Claude Code have no `agent_display_name` (F007); Copilot persists it in YAML when present and it stays out of Details and out of the Subagent cell; `task` still Copilot/Claude-absent (F006); omit absent fields.
...
- An ingest must **run as Node.js ≥ 24 ESM with no external dependencies**, including no YAML parsing library. The report must **accept** the Session YAML log as F003 writes it (fixed-structure multi-document key-value YAML). An ingest must **not migrate or rewrite** old YAML that still has `source_harness` / `source_event` / per-document `session_id`. New documents use the new keys. Reading mixed historical files is out of scope.

### Out of scope

- Reconstructing parent→subagent hierarchy or nesting subagents under a parent (`agent_id` is not in the YAML). An ingest must **not** copy `subagent` onto later documents that omit it.
...
- Changing F003 YAML append-only rules, YAML body mapping, or [`docs/normalized-fields.md`](../../normalized-fields.md). Reading `turn` from the F003 header (F008) and grouping the event list by turn are in scope for this spec as F008’s report consumer. YAML already has `subagent`, `agent_display_name`, `task`, and `response_text`; this amend only changes how the report displays them (F009: Subagent cell is the bare `subagent` value). Report labels follow YAML `harness` / `event`.
- Migrating or rewriting old YAML that still has `source_harness` / `source_event` / per-document `session_id`. Reading mixed historical files.
...
- Include overview (`session_id` from the F001 identifier / filename stem, `harness` from the last document, start, end, duration `HH:MM:SS` from first→last timestamps regardless of `event`), counts by `event` (session-level), and one Markdown subsection per turn that appears (turn number, duration, prompt preview for turns ≥ 1, then a flat Time / Event / Subagent / Details table of that turn’s documents in file order; Event is YAML `event`; Subagent is the bare `subagent` value when that document has the field; previews truncated at 100 characters with `...`).
