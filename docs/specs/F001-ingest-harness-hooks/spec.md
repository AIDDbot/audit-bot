...
Cursor, Claude Code, and GitHub Copilot each emit hook events (session start/end, prompts, tool use, durations) over stdin JSON, but nothing in this project captures them. Developers cannot audit agent sessions in one place. The CLI is still a health tracer.
...
- An Event must **identify the harness**, record **when it was received** (ISO 8601), record the **hook event name**, and **preserve the original stdin payload**.
...
- A project registration must **subscribe at least** to session start/end, prompt submit, pre/post tool use, tool-use failure, and stop (duration-bearing where the harness provides it):
  ...
  | After tool | `postToolUse` (`duration`) | `PostToolUse` | `postToolUse` |
  | Tool failure | `postToolUseFailure` (`duration`) | `PostToolUseFailure` | `postToolUseFailure` |
  | Turn stop | `stop` | `Stop` | `agentStop` |

  Copilot PascalCase names (`SessionStart`, `PreToolUse`, …) are the VS Code-compatible alias; this spec uses the CLI camelCase names. Extra harness events may be subscribed later; if they are, the same observe-only and JSONL rules apply.
...
- PII/secret redaction (payloads may include prompts, paths, emails, tool output).
...
An Event is one JSONL record: harness identity, received-at timestamp, hook event name, original stdin JSON. Session identity is whatever the payload already carries (`conversation_id` / `session_id` / `sessionId`).
...
- [ ] **AC-F001.2** — THE SYSTEM SHALL ingest required events from Cursor, Claude Code, and GitHub Copilot (session start/end, prompt submit, pre/post tool use, tool-use failure, stop).
...
- [ ] **AC-F001.6** — THE SYSTEM SHALL include in each stored Event the harness identity, an ISO 8601 received-at timestamp, the hook event name, and the original payload.
...
> last updated: 2026-08-31T18:12:00Z
