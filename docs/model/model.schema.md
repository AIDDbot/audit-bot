...
- **Session report** is the per-session Markdown file (`{session_id}.md` in the daily folder) overwritten on every later YAML append for that session the same day. The report groups events by each document’s `turn`.
