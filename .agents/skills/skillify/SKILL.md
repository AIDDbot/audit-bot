...
Write `SKILL.md` from the [skill template](./assets/skill.template.md), not from a sibling skill — siblings may still use the older house prose. Add whatever `references/` or `assets/` the skill needs.

When behavior or paths change, sync [`skills.catalog.md`](../skills.catalog.md). Touch `docs/` only when what a human is told changes.
