...
If you were given no plan, make one on the fly. Work on `feat/{spec_key}`, `chore/{spec_key}`, or `fix/{slug}`. Write the smallest change that resolves each task, check it off, and note any deviation. Lint, build, and unit-test where they apply. When a spec is in scope, set it to `in-progress`.
